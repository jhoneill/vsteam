...
Describe 'VSTeamWorkItemPage' {
   BeforeAll {
      ...
      Mock _callApi { return ([pscustomobject]@{name='Dummy'}) }  -ParameterFilter {$method -eq 'patch'}
   }

   Context 'Set-VSTeamWorkItemPage' {

      It 'should call Get-WorkitemType with -exapnd, unlock the WIT and call the PATCH API with the right body' {
         ## Act
         $wip = Set-VSTeamWorkItemPage -ProcessTemplate Scrum5 -WorkItemType Change* -label "Customer"  -Order 1 -Newlabel "Client" -force
         Should -Invoke Get-VSTeamWorkItemType -Scope It -ParameterFilter {
            $workitemtype -eq "Change*" -and
            $processTemplate -eq "scrum5" -and
            $expand -eq "layout"
         } -Times 1 -Exactly
         Should -Invoke unlock-VSTeamWorkItemType -Scope It -Times 1 -Exactly -ParameterFilter {$expand -eq "layout"}
         Should -Invoke _callapi -Scope It -parameterFilter {
               $method -eq     'patch' -and
               $url    -like   '*changeReq/layout/pages*' -and
               $body    -match '"label":\s*"Client"' -and
               $body    -match '"order":\s*1'
         } -Times 1 -Exactly
         ## Assert
         $wip.count | should -BeExactly  1
         $wip.psobject.TypeNames| should -contain 'vsteam_lib.WorkItemPage'
         $wip.WorkItemType      | should -BeExactly 'ChangeReq'
